  ...
  function refreshStyleSheets(theme, root = document) {
    ...
    for (const sheet of sheets) {
      ...
      if (!handle?.isConnected) {
        ...
        if (parent && owner.parentNode === parent) {
          try { parent.insertBefore(handle, owner.nextSibling); } catch { (document.head || document.documentElement).append(handle); }
        } else (document.head || document.documentElement).append(handle);
    ...
    for (const shadow of shadowRoots) {
      for (const sheet of [...(shadow.styleSheets || [])]) {
        ...
        if (!handle?.isConnected) {
          ...
          const parent = owner.parentNode;
          if (parent && owner.parentNode === parent) {
            try { parent.insertBefore(handle, owner.nextSibling); } catch { (shadow || document.head || document.documentElement).append(handle); }
          } else (shadow || document.head || document.documentElement).append(handle);
          sheetHandles.set(sheet, handle);
        }
        const cssText = overrides.join('\n');
        if (handle.textContent !== cssText) handle.textContent = cssText;
...
EXP.DynamicEngine = (() => {
  ...
  const cache = new Map();
  const stats = { runs:0, sheets:0, rulesSeen:0, rulesGenerated:0, inaccessible:0, remoteSheets:0, remoteRules:0, remoteFailures:0, cacheHits:0, cacheMisses:0, variables:0, groups:0 };
  let observer=null, timer=0, active=false, theme=null;
  ...
  function role(property,name=''){
    const key=`${property} ${name}`.toLowerCase();
    if(/background|\bbg\b|surface|canvas|panel|card|layer|container/.test(key))return'background';
    if(/color|text|foreground|\bfg\b|label|ink|content|fill|lighting/.test(key))return'foreground';
    if(/border|outline|divider|stroke|rule|separator/.test(key))return'border';
    return null;
  }
  ...
  function transformValue(property,value,vars,background){
    ...
    if(/url\s*\(/i.test(resolved)&&!/gradient\s*\(/i.test(resolved))return value;
    if(/gradient\s*\(/i.test(resolved)) return resolved.replace(/(?:#(?:[0-9a-f]{3,8})\b|rgba?\([^)]*\)|hsla?\([^)]*\))/ig,c=>EXP.ColorEngine.transform(c,'background',theme,background));
    ...
    const r=role(property);if(!r)return value;
  ...
  function walk(rules,out,vars=new Map(),budget=5000){
    if(!rules||stats.rulesSeen>=budget)return;
    ...
    for(const rule of rules){
      if(stats.rulesSeen>=budget)break;stats.rulesSeen++;
      try{
        if(rule.type===CSSRule.STYLE_RULE&&rule.selectorText&&rule.style){
          ...
          for(let i=0;i<rule.style.length;i++){
            ...
            if(p.startsWith('--')){const rr=role('',p);if(rr){next=resolve(v,scope).replace(/(?:#(?:[0-9a-f]{3,8})\b|rgba?\([^)]*\)|hsla?\([^)]*\))/ig,c=>EXP.ColorEngine.transform(c,rr,theme,bg));if(next!==v)stats.variables++;}}
  ...
  async function processRemoteSheet(sheet,root){
    ...
    const key=`${href}|${themeKey(theme)}`;let css=remoteCache.get(key);
    if(css===undefined){
      try{
        const source=rewriteUrls(await requestText(href),href),rules=parseRemote(source),out=[];
      ...
      }catch(error){stats.remoteFailures++;EXP.Core.safeError(Object.assign(error,{code:'REMOTE_STYLESHEET'}),'shift-dynamic');return;}
  ...
  function refresh(nextTheme){
    if(!active||!nextTheme)return;theme=nextTheme;stats.runs++;stats.sheets=stats.rulesSeen=stats.rulesGenerated=stats.inaccessible=stats.remoteSheets=stats.remoteRules=stats.remoteFailures=stats.variables=stats.groups=0;
    const roots=[document];document.querySelectorAll('*').forEach(el=>{if(el.shadowRoot)roots.push(el.shadowRoot);});
    const live=new Set();
    for(const root of roots){
...
EXP.Engine = (() => {
  ...
  function scheduleStyleSheetRefresh() {
    ...
    sheetRefreshTimer = setTimeout(() => {
      ...
      try {
        ...
        if (EXP.DynamicEngine) EXP.DynamicEngine.refresh(theme);
        else EXP.ColorEngine.refreshStyleSheets(theme);
  ...
  function scanRoots(roots, overrideLevel, deferredPass = false) {
    ...
    else { EXP.DynamicEngine?.stop(); EXP.ColorEngine.clearStyleSheets(); }
  ...
  function health() {
    ...
    return {
      ...
      dynamicEngine: EXP.DynamicEngine?.health?.() || null,
...
EXP.VERSION = '3.3.0-dev.3';

EXP.ReleaseNotes = (() => {
  const NOTES = Object.freeze({
    '3.3.0-dev.3': [
      'Introduces Dynamic Engine V2 as the primary stylesheet renderer.',
      'Adds fallback retrieval and transformation for inaccessible external stylesheets.',
      'Resolves relative asset URLs before processing remotely retrieved CSS.',
      'Keeps the existing DOM renderer as an exception-repair fallback.',
    ],
    '3.3.0-dev.2': [
